...
**John 17:3 hard-output floor:** When the prompt contains the Trinitarian reply pattern "the Father is the only true God, but not only the Father," the 2+2 only-placement analogy, John 1:1 / 1 John 5:20 proof-text stacking, and eternal-life entailment from knowing Jesus, do not close with the compact DO-12 floor alone. If the response claims `coverage_complete=true`, it should not emit fewer than six burden-level cells and fourteen owner-bearing submoves for this exact hard-output family, and the visible governed artifact should pay hard-output prose mass rather than compressing those rows below the retained-scope floor. Byte-floor under-mass is a proof-corpus harness/prose-mass failure, not a semantic reason to claim STOP; do not convert the floor into `coverage_complete=false` unless a real semantic burden remains unresolved. The minimum burden-cell partition is: (1) model-family / Father-only predicate / person-nature transfer; (2) only-placement and 2+2 analogy; (3) John 1:1 / 1 John 5:20 proof-text source-order stack; (4) eternal-life entailment and sender/sent relation; (5) worship/lordship referent and tawhid restoration; (6) post-land sacred-doctrine / broader-system immunity recoil generated by MRP. Do not merge the worship/lordship referent into the eternal-life cell when it is asserted or entailed, and do not absorb the post-land "sacred doctrine" recoil into the previous cell. Visible Layer B must separately execute: `do-christian-extensions.model-identification` on the operative or missing Trinitarian model family, explicitly discriminating Social Trinitarianism, Latin/psychological Trinitarianism, relative-identity approaches, mystery/apophatic deferral, and modalism/tritheism risk; `M9-predication-mode.md` on `God`, `true God`, `Father`, `Son`, `one`, `three`, `is God`, and the sender/sent relation; `do-attribute-precision.attribute-precision` as a separate owner-bearing operation over person/nature/attribute transfer with a delta such as `person-nature-transfer-blocked` or `attribute-precision-typed`; definition/analogy discipline for the only-placement and 2+2 move; source/authority-order repair on the proof-text stack, using concrete source delta tokens such as `proof-text-sorted`, `proof-text-hidden-support-blocked`, `hidden-support-blocked`, or `source-function-bounded`, not bare `source-order-repaired`; consequence tracing on the eternal-life entailment; worship/lordship referent repair plus tawhid restoration; and the generated recoil boundary. Each owner-bearing submove should include enough visible Target, Operation, Result/state-change, Contribution-to-Land, and TTP Operation Body prose for a cold reader and the Output Grapher cells to see what pressure was handled and how it landed. If the model family is not specified, that missing discriminator remains a live burden; it is not solved by generic "Trinity" wording.
