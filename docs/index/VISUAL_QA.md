...
- Select grouped theory cards by runtime phase.
- Verify cards remain buttons, focusable, and update notation highlighting.
- Verify the Theory rendering is not a duplicate Architecture pipeline.

Reference selected document:

- Focal object: selected generated source snapshot.
- Verify summary counts, search/filter controls, document list, and selected preview appear before the raw table.
- Verify the full generated source map is collapsed by default.
...
- Verify notation chips, formulas, and the Highlighted notation panel wrap without replacing formal notation with simplified labels.
