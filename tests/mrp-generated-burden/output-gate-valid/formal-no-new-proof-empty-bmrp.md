...
- B_LA = [B1, B2, B3]
- B_MRP = []
- live noetic burden: ¹B / selected boundary pressure.

## Burden 1 / ¹B: Selected boundary

Matched owner/TTP route: [P7]
- ACT records:
  ⟦ACT ¹B₁[P7.scope-boundary] :: π=selected-boundary-pressure :: body_ref=¹B₁ :: Δ=Δ¹B:scope-boundary-named :: Land(¹B)+⟧

Layer B — Governed Operation Body

¹B₁[P7] — name the selected boundary
- Target: selected-boundary-pressure that treats a typed remainder as bounded only after the formal reread.
- Operation: P7.scope-boundary: define the stop condition, held-route boundary, and reopen condition.
- Result/state-change: scope-boundary-named / the typed boundary is named and its escape route is explicitly checked.
- Contribution-to-Land(¹B): Land(¹B) is licensed because P7 names the stop condition, held-route boundary, and reopen condition, and the formal reread records no live generated edge.

TTP Operation Body:
P7 defines the stop condition, the held-route boundary, and the reopen condition. The output does not rely on topic words; the proof boundary is discharged in the structured no-new-resultant proof below.

Land(¹B): selected boundary named and landed.

[Mid-Reread Pressure]
Target: ¹B / selected boundary
R(H,Δ): held routes rechecked: selected typed remainder; live remainder: none; release/next: STOP.
Landed delta: Δ¹B / scope-boundary-named.
Pressure activations:
- freeze-landed-move: terminal-state accounting — freeze ¹B.
- dependency-tug: pressure class: cleared — typed remainder checked and not live.
- hidden-framework-recoil: pressure class: cleared — no public route opened.
...
```json
{
  ...
  "normalized_activation_record": {
    "n_frame": "selected-proof-boundary",
    "source_status": "hard-source",
  ...
  "owner_activations": [
    {
      ...
      "owner": "P7",
      "operation": "scope-boundary",
      "pressure": "selected-boundary-pressure",
      "body_ref": "¹B₁",
