...
def _usage_chain_errors(cycle_root: Path, ledger_ref: object, receipt_ref: object, identity: dict) -> list[dict[str,str]]:
    ...
    if transaction.get("kind") not in {"settlement","settlement-set","orphan-recovery","orphan-recovery-set"} or transaction.get("candidate_id")!=identity["candidate_id"] or transaction.get("cycle_or_review_batch_id")!=identity["matrix_id"] or transaction.get("campaign_authorization_sha256")!=identity["campaign_authorization_sha256"] or transaction.get("authorization_sha256")!=identity["matrix_authorization_sha256"] or transaction.get("cohort")!="gpt-producer" or transaction.get("lane")!="producer" or expected_calls!=5 or accounted!=5 or transaction.get("completed")!=5 or transaction.get("attempted")!=5 or transaction.get("unknown")!=0 or any(row.get("status")!="COMPLETED" or row.get("acknowledgment_origin")=="NONE" for row in provider_rows):return [_err("structural_usage_binding","settlement transaction does not bind five positively acknowledged completed producer calls and cycle/candidate/authorization identity")]
