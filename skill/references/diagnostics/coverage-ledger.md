...
> release-state note: this ledger now describes the committed GitHub baseline through Session 9. Local workspace artifacts such as `production-hardening-audit.md` remain non-governing unless tracked in git.
> current-pass note: 2026-04 commit-parity hardening pass — reconciled this ledger against committed GitHub `main`, removed stale "local-only" narration for already committed Session 8 / Session 9 changes, and re-aligned the excerpt-discipline rating to committed reality
...
These counts now describe the committed baseline through Session 9, with this Session 10 pass updating parity wording and recency notes only.
